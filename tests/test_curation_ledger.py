...
def test_latest_delta_is_empty_without_recorded_runs(tmp_path) -> None:
    _, ledger, _, _ = _services(tmp_path, [])

    delta = ledger.latest_delta()

    assert delta.status == "EMPTY"
    assert delta.current_run is None
    assert delta.previous_run is None
    assert delta.metrics is None
    assert delta.external_actions == []


@pytest.mark.asyncio
async def test_latest_delta_requires_two_runs_for_transitions(tmp_path) -> None:
    combined, ledger, _, _ = _services(
        tmp_path,
        [
            _opportunity(
    ...
    assert delta.status == "BASELINE_ONLY"
    assert delta.current_run is not None
    assert delta.current_run.run_id == run.run_id
    assert delta.previous_run is None
    assert delta.metrics is None


@pytest.mark.asyncio
async def test_latest_delta_compares_exact_recorded_memberships(tmp_path) -> None:
    combined, ledger, _, _ = _services(
        tmp_path,
        [
            _opportunity(
    ...
    assert delta.current_run is not None
    assert delta.previous_run is not None
    assert delta.current_run.run_id == second.run_id
    assert delta.previous_run.run_id == first.run_id
    assert delta.metrics is not None
    assert delta.metrics.new_opportunity_count.previous == 1
    assert delta.metrics.new_opportunity_count.current == 0
    assert delta.metrics.new_opportunity_count.change == -1
    assert delta.metrics.publishable_count.change == -1
    assert delta.entered_publishable_ids == []
    assert delta.exited_publishable_ids == ["greenhouse:1"]
    assert delta.published_only_in_current_run_ids == []
