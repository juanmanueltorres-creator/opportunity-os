...
class DailyCurationService:
    ...
    def run(
        ...
        *,
        ...
        post_preview_queue = self.queue_service.build(
            now=generated_at,
            policy=full_queue_policy,
        )
        if (
            post_preview_queue.model_dump(mode="json", exclude_none=False)
            != full_queue.model_dump(mode="json", exclude_none=False)
        ):
            raise RuntimeError(
